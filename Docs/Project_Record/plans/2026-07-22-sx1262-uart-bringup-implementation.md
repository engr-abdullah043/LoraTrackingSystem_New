...
- [x] Add `Drivers/SX126x/Inc` to both Debug and Release include paths in `.cproject`; do not edit generated `Debug/*.mk` files.
...
- [x] Define a context containing `hspi1`, NSS/BUSY/RESET ports and pins, bounded timeouts, last board error, and the last received status byte.
- [x] Implement Semtech-required functions with the exact `sx126x_hal.h` signatures: `sx126x_hal_write`, `sx126x_hal_read`, `sx126x_hal_reset`, and `sx126x_hal_wakeup`.
- [x] For every SPI transaction: wait for BUSY low with a deadline, drive NSS low, transfer command/data, always restore NSS high, and map failures to an inspectable board error.
...
- [x] Implement one `sx1262_bringup_run()` entry point returning a specific result enum; stop at the first failed operation.
...
- [x] Update status/changelog with implementation files, pinned driver version, build result, hardware transcript/result, and the next milestone (RF configuration only after bring-up passes).
...
The retained fake-HAL harnesses compile and link as ARM ELFs. They could not be executed because this machine has no native C compiler or ARM simulator; hardware execution remains the authoritative behavioral check.

## Completion Gate

- [ ] Host-side tests pass.
- [x] STM32CubeIDE clean build passes.
